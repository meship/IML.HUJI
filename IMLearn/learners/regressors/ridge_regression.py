...
import numpy as np
from IMLearn.learners.regressors.linear_regression import *
...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        LA_estimetor = LinearRegression(self.include_intercept_)
        if self.lam_ == 0:
            self.coefs_ = LA_estimetor.fit(X,y)
        else:
            mat = X @ X.T
            self.coefs_ = np.linalg.inv(mat + self.lam_ * np.identity(mat.shape[0])) @ X.T @ y
